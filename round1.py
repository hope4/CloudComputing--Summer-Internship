...
#Average stratergy
#taking average of number of instructions of HQ
...
AN= sum(Instructions)/len(Instructions)
AR= sum(RAM)/len(RAM)
AS= sum(SIZE)/len(SIZE) 

MN= min(Instructions)  #minimum stratergy
MR= min(RAM)
MS= min(SIZE)
...
for i in range(m):
        if AN <= Hmips[i] and AR <= Hram[i] and AS <= Hsize[i]:
                ...
                if (i+1)<=m-1:
                        AN=int(vmdict[1][0])+(Hmips[i+1]/2)
                        AR=int(vmdict[1][1])+(Hram[i+1]/2)
                        AS=int(vmdict[1][2])+(Hsize[i+1]/2)  
